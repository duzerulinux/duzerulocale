...
# i18n
APP = 'duzerulocale'
LOCALE_DIR = "/usr/share/duzeru/locale"
...
class DuzeruLocale:
    ...
    def __init__(self, show_input_methods):
        ...
        self.builder.set_translation_domain("duzerulocale")
        self.builder.add_from_file('/usr/lib/duzeru/duzeruLocale/duzeruLocale.ui')
    ...
    def read_im_info(self):
        ...
        im_info_path = "/usr/lib/duzeru/duzeruLocale/iminfo/{0}.info".format(self.current_language.split(".")[0].split("_")[0])
        if not os.path.exists(im_info_path):
            im_info_path = "/usr/lib/duzeru/duzeruLocale/iminfo/other.info"
    ...
    def set_minority_language_flag_path(self, locale_code, flag_path):
        ...
        if language_code == 'ca':
            flag_path = '/usr/share/duzeru/duzeruLocale/flags/16/_Catalonia.png'
        elif language_code == 'cy':
            flag_path = '/usr/share/duzeru/duzeruLocale/flags/16/_Wales.png'
        elif language_code == 'eu':
            flag_path = '/usr/share/duzeru/duzeruLocale/flags/16/_Basque Country.png'
        elif language_code == 'gl':
            flag_path = '/usr/share/duzeru/duzeruLocale/flags/16/_Galicia.png'
    ...
    def build_lang_list(self):
        ...
        self.countries = {}
        file = open('/usr/lib/duzeru/duzeruLocale/countries', "r")
        for line in file:
            line = line.strip()
            split = line.split("=")
            if len(split) == 2:
        ...
        self.languages = {}
        file = open('/usr/lib/duzeru/duzeruLocale/languages', "r")
        for line in file:
            line = line.strip()
            split = line.split("=")
            if len(split) == 2:
        ...
        for line in locales.split("\n"):
            ...
            if "_" in locale_code:
                ...
                if len(split) == 2:
                    ...
                    flag_path = '/usr/share/duzeru/duzeruLocale/flags/16/' + country_code + '.png'
            else:
                if locale_code in self.languages:
                    language_label = self.languages[locale_code]
                else:
                    language_label = locale_code
                flag_path = '/usr/share/duzeru/duzeruLocale/flags/16/languages/%s.png' % locale_code
            ...
            if os.path.exists(flag_path):
                ...
                flag = '/usr/share/duzeru/duzeruLocale/flags/16/generic.png'
    ...
    def set_user_locale(self, path, locale):
        ...
        if os.path.exists(self.pam_environment_path):
            ...
            os.system("sed -e 's/$locale/%s/g' -e 's/$region/%s/g' /usr/lib/duzeru/duzeruLocale/default_pam_environment.template > %s" % (locale.id, self.current_region, self.pam_environment_path))
    ...
    def set_user_region(self, path, locale):
        ...
        if os.path.exists(self.pam_environment_path):
            ...
            os.system("sed -e 's/$locale/%s/g' -e 's/$region/%s/g' /usr/lib/duzeru/duzeruLocale/default_pam_environment.template > %s" % (self.current_language, locale.id, self.pam_environment_path))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "im":
        print ("Starting duzerulocale in IM mode")
        show_input_methods = True
    else:
        print ("Starting duzerulocale")
        show_input_methods = False

    DuzeruLocale(show_input_methods)
    Gtk.main()
